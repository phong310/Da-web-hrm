...
import { styleDialogCategoryManagement } from '../Department/ModalCreateDepartment'
...
const ModalCreateTitles: React.FC<TitleType> = ({
  openModal,
  idEdit,
  closeModalDetail,
  onSuccess
  ...
  const { createOrUpdateApi } = useApiResource<BaseMaster>(`${V1}/admin/title`)
  ...
  useQuery<TitleResponse>([`${V1}/admin/title/${idEdit}`], {
    onSuccess: (data) => {
    ...
    <Dialog open={openModal} sx={{ ...styleDialogCategoryManagement }}>
      <AppBar sx={{ position: 'relative' }}>
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between' }}>
          <Typography>
            {isEdit ? t('titles_admin.edit_page') : t('titles_admin.create_page')}
          </Typography>
          ...
          >
            <Grid item container xs={12} columnSpacing={2}>
              <Grid item xs={12}>
                <Input
                  fullWidth
                  placeholder={t('titles_admin.name')}
                  label={t('titles_admin.name')}
                  name="name"
                  control={control}
                  required
                />
...
export default ModalCreateTitles
