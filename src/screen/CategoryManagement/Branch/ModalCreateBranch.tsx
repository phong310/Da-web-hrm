...
const ModalCreateBranch: React.FC<BranchType> = ({
  openModal,
  idEdit,
  closeModalDetail,
  onSuccess
  ...
  const { createOrUpdateApi } = useApiResource<BaseMaster>(`${V1}/admin/branch`)
  ...
  useQuery<BrachResponse>([`${V1}/admin/branch/${idEdit}`], {
    onSuccess: (data) => {
    ...
    >
      <AppBar sx={{ position: 'relative' }}>
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between' }}>
          <Typography>
            {isEdit ? t('branch_admin.edit_page') : t('branch_admin.create_page')}
          </Typography>
          ...
          >
            <Grid item container xs={12} columnSpacing={2}>
              <Grid item xs={12}>
                <Input
                  fullWidth
                  placeholder={t('branch_admin.name')}
                  label={t('branch_admin.name')}
                  name="name"
                  control={control}
                  required
                />
