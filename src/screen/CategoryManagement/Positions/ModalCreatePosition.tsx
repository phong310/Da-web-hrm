...
const ModalCreatePosition: React.FC<JobType> = ({
  openModal,
  idEdit,
  closeModalDetail,
  onSuccess
  ...
  const { createOrUpdateApi } = useApiResource<BaseMaster>(`${V1}/admin/position`)
  ...
  useQuery<PositionResponse>([`${V1}/admin/position/${idEdit}`], {
    onSuccess: (data) => {
    ...
    <Dialog open={openModal} sx={{ ...styleDialogCategoryManagement }}>
      <AppBar sx={{ position: 'relative' }}>
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between' }}>
          <Typography>
            {isEdit ? t('position_admin.edit_page') : t('position_admin.create_page')}
          </Typography>
          ...
          >
            <Grid item container xs={12} columnSpacing={2}>
              <Grid item xs={12}>
                <Input
                  fullWidth
                  placeholder={t('position_admin.name')}
                  label={t('position_admin.name')}
                  name="name"
                  control={control}
                  required
                />
